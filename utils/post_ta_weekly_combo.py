...
def post_ta_weekly_combo():
    ...
    """
    logging.info("🚀 Starting post_ta_weekly_combo")

    # 1) Generate the Substack article markdown
    logging.info("📝 Generating weekly TA Substack article")
    filepath = generate_weekly_ta_article()
    if not filepath or not os.path.exists(filepath):
        logging.error("❌ TA article generation failed; aborting combo.")
        return
    ...
    with open(filepath, "r", encoding="utf-8") as f:
        lines = f.readlines()
    if not lines:
        logging.error("❌ Generated markdown is empty; aborting.")
        return
    ...
    try:
        logging.info("📚 Publishing TA article to Substack")
        ...
        logging.info(f"✅ Published TA article at {substack_url}")
    except Exception as e:
        logging.error(f"❌ Failed to publish TA article: {e}")
        return
    ...
    try:
        ...
        logging.info("📢 Posting weekly TA update on X")
        post_thread([message], category="ta_weekly")
        logging.info("✅ Weekly TA update posted on X")
    except Exception as e:
        logging.error(f"❌ Failed to post TA update on X: {e}")
