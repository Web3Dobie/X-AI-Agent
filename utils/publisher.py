...
def publish_and_announce(generate_fn, publish_fn, announce_fn, announce_args_builder):
    ...
    """
    logging.info("🚀 Starting publish_and_announce workflow")
    try:
        data = generate_fn()
        logging.info("✅ Content generated successfully")
    except Exception as e:
        logging.error(f"❌ generate_fn failed: {e}")
        return
    ...
    try:
        if isinstance(data, tuple):
            ...
            url = publish_fn(data)
        logging.info(f"✅ Published content, received URL: {url}")
    except Exception as e:
        logging.error(f"❌ publish_fn failed: {e}")
        return
    ...
    try:
        ...
        logging.error(f"❌ announce_fn failed: {e}")
